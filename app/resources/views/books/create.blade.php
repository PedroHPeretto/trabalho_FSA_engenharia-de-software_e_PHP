...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        <h1 class="text-xl font-bold text-gray-900 mb-6">Novo Livro</h1>
        ...
        <form method="POST" action="{{ route('books.store') }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <select id="media" name="media" onchange="toggleMediaFields(this.value)"
                    ...
                    <option value="physical" @selected(old('media') === 'physical')>Físico</option>
                    <option value="digital" @selected(old('media') === 'digital')>Digital</option>
                </select>
                @error('media')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
            </div>

            <div id="field-stock" class="{{ old('media') === 'physical' ? '' : 'hidden' }}">
            ...
            <div id="field-digital-link" class="{{ old('media') === 'digital' ? '' : 'hidden' }}">
                <label for="digital_link" class="block text-sm font-medium text-gray-700 mb-1">Link Digital</label>
                <input type="url" id="digital_link" name="digital_link" value="{{ old('digital_link') }}"
                       placeholder="https://..."
            ...
            <div id="field-pdf" class="{{ old('media') === 'digital' ? '' : 'hidden' }}">
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('books.index') }}" class="border border-gray-300 hover:bg-gray-50 px-6 py-2 rounded-lg text-sm transition">
                    Cancelar
                </a>
            </div>
        </form>
